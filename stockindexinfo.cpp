 ...
 * 类静态方法
 * 将两个自定义的格式字符串合并为一个
 * 第一、二个参数为两个待合并的字符串，第三个参数为合并后的输出
 * StockIndexBatchInfo字符串格式如下；
 * %ts_code:trade_date,currPrice;trade_date,currPrice;
 * ts_code格式类似于：000001.SZ
 */
void StockIndexBatchInfo::mergeTwoEncodeStr(std::string& in1, std::string& in2, std::string& out) {
    ...
    out.append(in2.substr(mainStartIndex + 1));
}
